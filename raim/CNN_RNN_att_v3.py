...
def get_model_lstm():
    ...
    encoded_sequence = Bidirectional(LSTM(100, return_sequences=True))(encoded_sequence)
    attention_probs  = TimeDistributed(Dense(200, activation='softmax'))(encoded_sequence)
    merged = keras.layers.multiply([encoded_sequence, attention_probs])
    out = Convolution1D(nclass, kernel_size=1, activation="softmax", padding="same")(merged)
